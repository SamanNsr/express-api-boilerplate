...
describe('Error middlewares', () => {
    ...
    test('should convert an Error without status to ApiError with status 500', () => {
      ...
      expect(next).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          message: error.message,
          isOperational: false,
          textId: 'messages.general.internalServerError',
        }),
      );
    });
    ...
    test('should convert an Error without message to ApiError with default message of that http status', () => {
      ...
      expect(next).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          message: 'BAD_REQUEST',
          isOperational: false,
          textId: 'messages.general.badRequest',
        }),
      );
    });
  ...
  describe('Error handler', () => {
    ...
    test('should send proper error response and put the error message in res.locals', () => {
      const error = new ApiError(httpStatus.BAD_REQUEST, httpStatus.BAD_REQUEST, 'Any error');
